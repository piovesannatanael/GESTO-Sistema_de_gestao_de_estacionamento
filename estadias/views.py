...
class EstadiaListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        elif status_filter == 'finalizadas':
...
class EstadiaChegadaUpdateView(LoginRequiredMixin, PermissionRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        estadia_antiga = self.get_object()  # Vaga anterior antes da edição
        vaga_antiga = estadia_antiga.vaga

        # Salva a nova estadia
        estadia = form.save()
        vaga_nova = estadia.vaga


        if vaga_antiga != vaga_nova:
            if vaga_antiga:
                vaga_antiga.status = 'livre'
                vaga_antiga.save()

            if vaga_nova:
                vaga_nova.status = 'ocupada'
                vaga_nova.save()
        messages.success(self.request, 'Estadia atualizada com sucesso!')
        return super().form_valid(form)
